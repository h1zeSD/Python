...
def save_to_txt_for_people(phones, filename):
    try:
    ...
    print(f"Телефоны успешно сохранены в файл {filename} в человекочитаемом виде")


def save_to_txt_for_comp(phones, filename):
    try:
        file_out = open(filename, "w", encoding="utf-8")
    except:
        print("Не удалось записать данные в файл")
...
save_to_txt_for_people(phones, "filename.txt")
save_to_txt_for_comp(phones, "filename.txt")
